...
async def main():
    ...
    try:
        ...
        await create_transcript(news, transcript, together_backend.summarize)
        ...
        # 1. MP3
        await add_to_db(url, f"{DATA_DIR}/{today_iso_fmt}_podcast_dist.mp3")

        # 2. Episode
        word_count = count_words(transcript)
        await add_episode_to_db(time_to_produce=production_time, word_count=word_count)

        # 3. Add articles to the episode
        await add_articles_to_episode()

        # Render the Jinja template
        await render_jinja_template(production_time, word_count)
